...
- **Post-deploy Docker artifact pruning is on by default** (May 2026, commit `eac1f918`). `deploy-docker.sh` runs `prune_old_docker_artifacts()` on the app host and on every entry in `WORKER_HOSTS` at the end of each deploy: container/image/builder prune unconditionally, and `docker volume prune -f` only after asserting `judgekit-db` is running (per `CLAUDE.md` "never `docker system prune --volumes`"). Rationale: every `--no-cache` worker-image rebuild leaves the prior layer set dangling, so unpruned hosts hit disk-full and the next deploy thrashes (`auraedu` stalled at 95 % / 6.6 GB free in May 2026 before this landed). Set `SKIP_POST_DEPLOY_PRUNE=1` to opt out for a single deploy.
