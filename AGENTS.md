...
- **Dedicated worker URL reconciliation** (2026-06-30 cycle 2). Split-host deploys (`WORKER_HOSTS`, e.g. `algo` → `worker-0.algo.xylolabs.com`; the retired `worv` target used `worker.test.worv.ai`) upsert `JUDGE_BASE_URL=<app AUTH_URL>/api/v1` into each worker host's `~/judgekit/.env` before restarting `docker-compose.worker.yml`. The deploy aborts when that URL would be non-local HTTP; production recovery must fix TLS / `AUTH_URL_OVERRIDE`, not set `JUDGE_ALLOW_INSECURE_HTTP=1`.
...
`oj.auraedu.me` and `algo.xylolabs.com`. Keep this aligned with
`docs/deployment.md` when rotating keys. The former `worv` target
(`test.worv.ai`, key `~/.ssh/worv-judgekit.pem`) was retired from the deploy
roster on 2026-07-06; `deploy-docker.sh` rejects `DEPLOY_TARGET=worv` and the
leftover local `.env.deploy.worv` / `.env.worv` files must not be used.
