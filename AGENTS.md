...
- **Pre-build disk guard + recurring host cleanup** (2026-06-19, after `algo.xylolabs.com` hit 100% root FS — a failed PowerShell language-image build filled the disk and left a broken deploy, because the build died *before* the post-deploy prune could run). Two layers: **(a)** `deploy-docker.sh` now runs a disk preflight right after "Remote docker verified": if the remote root FS is `>= DEPLOY_DISK_WARN_PCT` (default 85%) it reclaims **dangling images (`-f`, never `-af`) + build cache (`builder prune -af`) + the BuildKit history store (`docker buildx history rm --all`)** — NEVER volumes — then aborts with `die` if still `>= DEPLOY_DISK_HARD_PCT` (default 92%) rather than starting a doomed build. **(b)** A host-level `systemd` timer (`/etc/systemd/system/docker-disk-cleanup.{service,timer}` → `/usr/local/bin/docker-disk-cleanup.sh`, runs every 6h) prunes the same artifacts independent of deploys, so even failed/aborted deploys get cleaned. The cleanup script escalates to a full build-cache + history purge when disk `>= 80%`. **CRITICAL: neither path ever prunes volumes** (`docker volume prune` / `docker system prune --volumes` are absent by design — the PostgreSQL data volume lives there; this is the Apr 2026 data-loss class). The script uses `docker image prune -f` (dangling only) so it is safe on judge hosts too (tagged `judge-*` images are never wiped). Installed on `algo.xylolabs.com`; replicate on other Docker hosts with the same three files. Verified: a manual run left all 4 volumes and all tagged images intact.
