# Cycle 4 (2026-07-01) Deferred Findings Register

Source: `.context/reviews/_aggregate.md` (2026-06-30 cycle) and the per-agent review files under `.context/reviews/`.

This file records findings that are **explicitly deferred** from the current cycle. All CRITICAL/HIGH security, correctness, and data-loss items that remain unimplemented are scheduled in `plan/cycle-4-2026-07-01-review-remediation.md` Phase A. This register is for product roadmap gaps, UI/UX polish, performance tuning, documentation drift, E2E/test-quality work, and operational convenience that does not block production safety.
...
The following findings were explicitly dispositioned as accepted risk in the cycle-3 Rejected/Not-New Register (`plan/cycle-3-2026-06-30-nginx-env-hardening.md`). They are not scheduled or deferred as new work; they remain closed pending a policy or architecture change.
...
All CRITICAL/HIGH security, correctness, and data-loss items scheduled for implementation are instead recorded in `plan/cycle-4-2026-07-01-review-remediation.md` Phase A with concrete acceptance criteria. During the 2026-07-01 planning pass a broader A1-A34 draft was generated; the implemented cycle scope was reduced to the 12 critical/correctness stories in that remediation file. Remaining draft items — including `hcaptcha.ts` unit tests, `derive-key.ts` domain-separation tests, restoring the global nginx `client_max_body_size`, judge-route input validation hardening, admin settings re-confirmation coverage, and additional test-quality work — are retained in this register and scheduled for future cycles.
