...
- [ ] A1 contest export JSON audit unconditional
- [ ] A2 roles PATCH cannotEditHigherRole gate
- [ ] A3 updateRecruitingInvitation tx + FOR UPDATE
- [ ] A4 worker executor catch_unwind
- [ ] A5 runner.rs chown+0o700
- [ ] A6 SSE re-auth canAccessSubmission
- [ ] A7 community create+vote through helper
- [ ] A8 admin/settings PUT password reconfirm
- [ ] A9 deploy-docker.sh per-target env sourcing
- [ ] A10 LOW batch (freezeLeaderboardAt, accepted-solutions count, worker clamp warn, rankings ISR, PB-1 test name)
- [ ] A11 test-gap batch (migrate-import mirror, worker timeout coverage)
- [ ] A12 docs batch (CSRF doc, push-scan wording, AGENTS line ref, .env.example vars, X-Real-IP CI guard)
