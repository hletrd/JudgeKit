 ...
 *
 * PostgreSQL-specific: uses primary key for batch selection. Previously
 * used `ctid` but was changed to primary key for stability across
 * VACUUM operations. This approach is not portable to MySQL or SQLite —
...
async function batchedDelete(
  ...
  while (true) {
    const result = await db.execute(
      sql`DELETE FROM ${table} WHERE ${table.id} IN (SELECT ${table.id} FROM ${table} WHERE ${whereClause} LIMIT ${BATCH_SIZE})`
    );
