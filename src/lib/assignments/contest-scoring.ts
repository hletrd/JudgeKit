  ...
  function buildScoringQuery(withCutoff: boolean): string {
    return `
      ...
      SELECT
        ...
        MAX(CASE WHEN ROUND(score::numeric, 2) = 100 THEN 1 ELSE 0 END) AS "hasAc",
        MIN(CASE WHEN ROUND(score::numeric, 2) = 100 THEN submitted_at ELSE NULL END) AS "firstAcAt",
