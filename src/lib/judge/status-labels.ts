...
export function buildStatusLabels(t: (key: string) => string): Record<string, string> {
  return {
    ...
    time_limit_exceeded: t("status.time_limit"),
    memory_limit_exceeded: t("status.memory_limit"),
    output_limit_exceeded: t("status.output_limit_exceeded"),
    internal_error: t("status.internal_error"),
