...
**REDESIGN (2026-07-21).** The first implementation of this task (commit `34b4ad36`) failed review with four Criticals and its injection mechanism must be replaced. Read this section as authoritative; where it contradicts the Step 3 sketch below, this section wins.

**Governing principle — verdict parity.** A warm run and a cold run of the same submission MUST produce the same verdict. Any measurement or resource difference that could flip Accepted↔TLE or Accepted↔MLE depending on whether a warm container happened to be free is a defect, not a tradeoff.

1. **Replace tar/`docker cp` injection with a read-only bind-mounted staging directory.**
   `docker cp` is rejected outright by the daemon for `--read-only` containers, so the original mechanism could never have worked. Instead: when `create_warm_container` builds a container, first create a dedicated empty host staging directory for it and mount that directory at `/workspace` **read-only** (`-v <staging>:/workspace:ro`), replacing `WARM_WORKSPACE_TMPFS` entirely. At adopt time the worker writes the submission's files into that host directory from the host side — exactly the way the cold path already prepares `options.workspace_dir` — and the container sees them through the live bind mount. Reuse whatever host-path handling the cold path already relies on for its own workspace mount, since that is already proven to work under the docker-socket-proxy setup. Destroy the staging directory when the container is destroyed.
   This single change resolves three separate Criticals: permissions (the files keep the same ownership/modes the cold path uses, instead of being rewritten by a root tar extraction), memory accounting (a `:ro` bind mount's page cache is reclaimable, whereas tmpfs pages were charged unreclaimably against the submission's memory limit), and unbounded archive buffering in worker RAM. It also restores W^X for free, because the mount is genuinely read-only rather than locked down after the fact.

2. **A failed `docker exec` must never become a verdict.** Currently only `spawn()` errors map to `WarmUnavailable`; a non-zero exec *exit* is returned as a normal run result, so a container reaped between inspect and exec yields exit 126 and the submission is reported as a RuntimeError. Detect exec-machinery failure (e.g. exit 126/127 combined with `OCI runtime exec failed` / `No such container` on stderr and empty stdout) and return `WarmUnavailable` so the caller re-runs cold.

3. **Timing must not be inflated by exec overhead.** Cold derives `duration_ms` from the container's `StartedAt`→`FinishedAt`, deliberately excluding CLI and daemon overhead. An adopted container cannot use those timestamps, and wall-clock around the `docker exec` CLI adds roughly 50–150 ms — enough to turn a 950 ms submission under a 1000 ms limit into a warm-only TLE. Solve this so warm timing is comparable to cold. **Required safety net regardless of the approach chosen:** if the measured duration or peak memory lands close enough to the limit that measurement error could change the verdict, return `WarmUnavailable` and let the authoritative cold path decide. A borderline submission getting a cold re-run is cheap; a wrong verdict is not.

4. **Refusal conditions** (all return `WarmUnavailable`, never a submission failure): `options.memory_limit_mb` above `WARM_CEILING_MEMORY_MB` (`docker update` may only ever LOWER limits, never raise them past creation values); `options.needs_exec_tmp` languages (warm containers get the strict `RUN_TMPFS`, not the exec-allowed `COMPILE_TMPFS`); a missing or dead container; and any injection or exec-machinery failure.

5. **Do not block the feature on a report-only metric.** `memory_peak_kb` is reported but is never an input to a verdict. Refusing the entire warm path on kernels where `memory.peak` cannot be reset (5.19–6.7) trades a real feature for a cosmetic number — prefer degrading or annotating that field, and log the condition once.

6. **Tests must round-trip real fixtures.** The previous attempt's 18 tests were all argument-shape assertions, which is exactly why a fatal permissions bug shipped. Include at least one test that prepares a workspace fixture with the same ownership/modes the executor actually produces and asserts the container-visible result is readable by the run user and not writable.
