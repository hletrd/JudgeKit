...
For assignments where `examMode != "none"` AND `enableAntiCheat = true`, the submission API (`/api/v1/submissions` POST, via `validateAssignmentSubmission` in `src/lib/assignments/submissions.ts`) checks that the latest **client-emitted** `anti_cheat_events` row for `(userId, assignmentId)` is no older than 90 seconds (`ANTI_CHEAT_HEARTBEAT_FRESHNESS_MS`). The browser monitor emits a heartbeat every 30 s (`HEARTBEAT_INTERVAL_MS` in `src/components/exam/anti-cheat-monitor.tsx`); the anti-cheat route persists at most one heartbeat ROW per 60 s per (assignment, user), so the latest persisted row is at most ~60 s old for an honest client — leaving a >=30 s buffer for clock skew and network jitter. Two scoping rules keep the flag honest (RPF cycle-4, tightened cycle-5):
